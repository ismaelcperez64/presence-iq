...
const TABS: { key: Tab; label: string }[] = [
  ...
  { key: 'negative', label: 'Issues Found' },
    ...
    <div className="w-full max-w-3xl mx-auto animate-fade-in">
      ...
      <div className="rounded-xl border border-brand-border bg-brand-card p-6 mb-6 flex flex-col items-center gap-6 text-center">
        <ScoreGauge
          ...
          onVerdictClick={() => setShowScoreModal(true)}
        />
        <div className="w-full">
          <h3 className="text-lg font-bold text-white mb-2">Overall Digital Presence</h3>
          <p className="text-slate-400 text-sm leading-relaxed max-w-lg mx-auto">{result.summary}</p>
          <div className="flex flex-wrap gap-3 mt-4 justify-center">
            <StatCard
              label="Platforms Found"
              value={result.platforms.filter(p => p.found).length}
              total={result.platforms.length}
              onClick={() => setTab('platforms')}
            />
            <StatCard
              label="Issues Found"
              value={result.negativeContent.length}
              alert={result.negativeContent.length > 0}
              onClick={() => setTab('negative')}
            />
            <StatCard
              label="Copycats"
              value={result.copycats.length}
              alert={result.copycats.length > 0}
              onClick={() => setTab('copycats')}
            />
          </div>
        </div>
      </div>
...
function StatCard({
  ...
  return (
    <button
      onClick={onClick}
      className="bg-brand-border/50 rounded-lg px-3 py-2 text-center cursor-pointer hover:bg-brand-border transition-all hover:scale-105 min-w-[80px]"
    >
      <div className={`text-xl font-black ${alert && value > 0 ? 'text-red-400' : 'text-white'}`}>
        {total !== undefined ? `${value}/${total}` : value}
      </div>
      <div className="text-xs text-slate-500">{label}</div>
      <div className="text-xs text-brand-cyan/60 mt-0.5">view →</div>
    </button>
  );
}
